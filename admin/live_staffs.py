...
def live_staff_ai_appform_saved(staff_id):
    ...
    if rec:
        return jsonify({
            ...
            "found":        True,
            "ai_id":        str(rec["_id"]),
            "filename":     rec.get("filename", rec.get("gcs_blob","").split("/")[-1]),
            "gcs_blob":     rec.get("gcs_blob", ""),
            "generated_at": rec["generated_at"].strftime("%d %b %Y %H:%M") if rec.get("generated_at") else "",
        })
    return jsonify({"success": True, "found": False})
...
def live_staff_ai_appform_download(ai_id):
    try:
        ...
        if not rec or not rec.get('gcs_blob'):
            return jsonify({"success": False, "error": "App form not found"}), 404
        docx_bytes = _gcs_download(rec['gcs_blob'])
...
def live_staff_api_generate_cv():
    ...
    try:
        ...
        if not doc:
            return jsonify({"success": False, "error": f"Staff not found"}), 404
...
def live_staff_api_get_staff_details():
    ...
    data = _ser(dict(staff))
    data['_id'] = str(staff['_id'])

    documents = []; doc_url_map = {}
    xn_base  = os.environ.get('LIVE_STAFF_URL', '').rstrip('/')
    xn_key   = os.environ.get('XN_PORTAL_API_KEY', '')
    xn_ctry  = os.environ.get('XN_APP_COUNTRY', '')
    if xn_base and xn_key:
        try:
            hdrs = {"Api-Key": xn_key, "X-App-Country": xn_ctry,
                    "Content-Type": "application/json", "Accept": "application/json"}
            r = _req2.post(f"{xn_base}/ai/recruitments/user-document-list",
                           json={"email": email}, headers=hdrs, timeout=30)
            if r.status_code == 405:
                r = _req2.get(f"{xn_base}/ai/recruitments/user-document-list",
                              params={"email": email}, headers=hdrs, timeout=30)
            if r.status_code == 200:
                _d = r.json().get('data')
                documents = _d if isinstance(_d, list) else (_d.get('documents') or [] if isinstance(_d, dict) else [])
                doc_url_map = {_v(d.get('document_type_name') or ''): _v(d.get('url') or '')
                               for d in documents if d.get('document_type_name') and d.get('url')}
        except Exception:
            pass

    return jsonify({"success": True, "email": email, "data": data,
                    "documents": documents, "document_urls": doc_url_map})


# ── API: Staff documents (return URLs for local download) ─────────────

@admin_bp.route('/live-staffs/api/staff-documents', methods=['POST'])
def live_staff_api_staff_documents():
    """Return document URLs for a staff member from XN Portal."""
    ...
    col = _staffs_col(); doc = None
    if staff_id:
        try: doc = col.find_one({"_id": ObjectId(staff_id)})
        except Exception: pass
    if not doc and email:
        doc = col.find_one({"$or": [{"email": email},
                                    {"section_1_personal_details.email_address": email}]})
    if not doc:
        return jsonify({"success": False, "error": "Staff not found"}), 404
    ...
    base_url = os.environ.get('LIVE_STAFF_URL','').rstrip('/')
    api_key  = os.environ.get('XN_PORTAL_API_KEY','')
    country  = os.environ.get('XN_APP_COUNTRY','')
    hdrs = {"Api-Key": api_key, "X-App-Country": country,
            "Content-Type": "application/json", "Accept": "application/json"}

    try:
        r = _req3.post(f"{base_url}/ai/recruitments/user-document-list",
                       json={"email": email}, headers=hdrs, timeout=30)
        if r.status_code == 405:
            r = _req3.get(f"{base_url}/ai/recruitments/user-document-list",
                          params={"email": email}, headers=hdrs, timeout=30)
        r.raise_for_status()
        _d = r.json().get('data')
        raw = _d if isinstance(_d, list) else (_d.get('documents') or [] if isinstance(_d, dict) else [])
    except Exception as e:
    ...
    documents = []
    for d in raw:
        dtype = _v(d.get('document_type_name') or 'Unknown')
        url   = _v(d.get('url') or '')
        if not url: continue
        ext   = _ext(url)
        safe  = _re3.sub(r'[<>:"/\\|?*\s]+', '_', dtype)[:60]
        documents.append({"document_type_name": dtype, "url": url,
                          "filename": safe + ext, "extension": ext})

    return jsonify({"success": True, "email": email, "staff_name": name,
                    "staff_id": str(doc['_id']), "count": len(documents),
                    "documents": documents})


@admin_bp.route('/live-staffs/api/all-staff-emails', methods=['GET'])
def live_staff_api_all_emails():
    """Return list of all staff emails for local downloader."""
    cron_secret = os.environ.get('CRON_SECRET', '')
    if cron_secret:
        provided = (request.args.get('cron_key') or
                    request.headers.get('X-Cron-Key', ''))
    ...
    for s in staff:
        ...
        name  = _v(s1.get('full_name') or '')
        if email:
            result.append({"staff_id": str(s['_id']), "email": email,
                           "name": name, "code": _v(s.get('employee_code') or '')})
    return jsonify({"success": True, "count": len(result), "staff": result})
...
def live_staff_export():
    ...
    if fmt == 'csv':
        ...
        flat = []
        for d in docs:
            s = _ser(d); s1 = s.get('section_1_personal_details') or {}
            flat.append({'_id': str(d['_id']), 'full_name': s1.get('full_name',''),
                         'email': s.get('email',''), 'user_type': s.get('user_type',''),
                         'employee_code': s.get('employee_code',''),
                         'nationality': s1.get('nationality',''),
                         'nmbi_number': s.get('nmbi_number',''),
                         'qqi_number': s.get('qqi_number','')})
        ...
        return Response(out.getvalue(), mimetype='text/csv',
                        headers={"Content-Disposition": "attachment; filename=live_staffs.csv"})
    data = [dict(_ser(d), **{'_id': str(d['_id'])}) for d in docs]
    return Response(__import__('json').dumps(data, default=str, ensure_ascii=False),
                    mimetype='application/json',
                    headers={"Content-Disposition": "attachment; filename=live_staffs.json"})
