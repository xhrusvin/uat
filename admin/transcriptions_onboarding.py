# admin/transcriptions_onboarding.py
from flask import (
...
def onboarding_tr():
    ...
    return render_template(
        'admin/transcriptions_onboarding.html',
        convs=convs,
        page=page,
        total=total,
        per_page=per_page,
...
# ===============================
@admin_bp.route('/onboarding_tr/<conv_id>/audio')
@admin_required
...
@admin_bp.route('/elevenlabs/api/onboarding_tr/<conversation_id>/summary')
@admin_required
def elevenlabs_summary_proxy_onboarding_tr(conversation_id):
    api_key = os.getenv("ELEVENLABS_API_KEY")
    if not api_key:
...
# AUDIO ENDPOINT
# ===============================
@admin_bp.route('/elevenlabs/api/onboarding_tr/<conversation_id>')
@admin_required
def elevenlabs_onboarding_api_proxy(conversation_id):
    api_key = os.getenv("ELEVENLABS_API_KEY")
    if not api_key:
        return jsonify({"error": "API key missing"}), 500
...
@admin_bp.route('/api/onboarding_tr/<conv_id>/transcript')
@admin_required
def get_onboarding_transcript(conv_id):
