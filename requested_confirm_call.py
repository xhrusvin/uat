...
def register_requested_confirm_call_routes(app):
    ...
    def requested_confirm_call():
        ...
        if user_id:
            ...
            "is_admin": {"$ne": True},
            "$or": [
                {"requested_confirm_call": 0},
                {"requested_confirm_call": {"$exists": False}}
            ]
        }
        ...
        if user.get("requested_confirm_call") == 1 and not user_id:
            return jsonify({
                **response_base,
                "status": "already_sent",
    ...
    # --------------------------------------------------------------
    # 2. MANUAL TRIGGER: /requested_confirm_call/trigger/<user_id> (ALSO TIME-RESTRICTED)
    # --------------------------------------------------------------
    @app.route('/requested_confirm_call/trigger/<user_id>', methods=['POST'])
    def requested_confirm_call_trigger(user_id):
        allowed, server_time = is_within_call_window()
