...
ALLOWED_CAMPAIGNS = {
    "120235638775730662": "Theatre Nurse",                   # Original Theatre Nurse campaign
    "120228337783440662": "Nurse",                   # Additional Nurse campaign
    "120228353185220662": "Health Care Assistant",  # HCA campaign
    "120233187190450662": "Midwives",  # Midwives campaign
    "120234057545540662": "Xpress GP",  # Xpress GP campaign
}
...
def init_fb_lead_fetcher(db):
    ...
    log.info("[FB LEAD FETCHER] Ready – processing only IRELAND phone numbers from allowed campaigns")
...
def is_irish_phone(raw_phone):
    """Return True only if phone number is from Ireland (+353 or 353)"""
    if not raw_phone:
        return False
...
def fetch_leads_for_form(form_id, form_name):
    ...
    while url:
        try:
            ...
            for lead in data.get("data", []):
                ...
                # Filter 1: Must be from allowed campaign
                ...
                # Filter 2: ONLY Irish phone numbers allowed
                if not is_irish_phone(raw_phone):
                    filtered_out += 1
                    log.info(f"SKIPPED NON-IRISH → {full_name} | {raw_phone or 'No phone'} | Campaign: {campaign_id}")
                    continue

                # Clean phone: remove +, keep digits only
                ...
                doc = {
                    ...
                    "country": 1,  # Hardcoded to Ireland (ID 1)
                    "call_initiated": 0,
                ...
                try:
                    ...
                    if result.upserted_id:
                        inserted += 1
                        log.info(f"SAVED {user_type.upper()} → {full_name} | {email or '-'} | {phone} | Campaign: {campaign_id}")
                    else:
                        skipped += 1
                except Exception as e:
    ...
    log.info(f"Form {form_name}: Inserted={inserted}, Skipped/Duplicate={skipped}, Filtered Out={filtered_out}")
...
def register_fb_lead_routes(app):
    ...
    def fetch_fb_leads_endpoint():
        ...
        try:
            ...
            return jsonify({
                "status": "success",
                "message": "Facebook leads synced – ONLY IRELAND (+353) phone numbers from Nurse/HCA campaigns",
                "stats": stats,
