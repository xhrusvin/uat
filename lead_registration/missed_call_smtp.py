...
@bp.route("/send-missed-call-email", methods=["GET"])
...
@bp.route("/process-missed-call-email", methods=["GET"])
def process_missed_call_email():
    """
    ...
    "call_sent": {"$exists": True},
    "$or": [
        {"missed_call_email_sent": {"$ne": 1}},
        {"missed_call_email_sent": {"$exists": False}}
    ]
}
        ...
        if success:
            ...
            website_leads_collection.update_one(
                ...
                {"$set": {
                    ...
                    "missed_call_email_sent_at": datetime.utcnow().isoformat(),
