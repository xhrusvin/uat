...
def register_shift_booking_call_bulk_routes(app):
    ...
    # --------------------------------------------------
    @app.route('/shift_booking_call_bulk', methods=['GET'])
    def auto_shift_booking_call_bulk():
    ...
    # 4. DEBUG ENDPOINT
    # --------------------------------------------------
    @app.route('/debug-shift-booking-call-bulk')
    def debug_shift_booking_bulk():
        allowed, now = is_within_call_window()
        return jsonify({
            "debug": "shift_booking_call_bulk.py loaded",
