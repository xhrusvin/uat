...
from contactpreferencecall import make_contact_preference_call
...
def register_contact_preference_call_routes(app):
    """Register /contact_preference_call → auto-trigger only 8 AM – 8 PM UTC."""

    # --------------------------------------------------------------
    # 1. AUTO-TRIGGER: /contact_preference_call (TIME-RESTRICTED)
    # --------------------------------------------------------------
    @app.route('/contact_preference_call', methods=['GET'])
    def contact_preference_call():
        ...
        if user_id:
            ...
            "is_admin": {"$ne": True},
            "$or": [
                {"contact_preference_call_sent": 0},
                {"contact_preference_call_sent": {"$exists": False}}
            ]
        }
        ...
        if user.get("contact_preference_call_sent") == 1 and not user_id:
            return jsonify({
                **response_base,
                "status": "already_sent",
        ...
        result = app.db.users.update_one(
            {"_id": user_id},
            {"$set": {"contact_preference_call_sent": 1, "updated_at": datetime.utcnow()}}
        )
        ...
        threading.Thread(
            target=make_contact_preference_call,
    ...
    # --------------------------------------------------------------
    # 2. MANUAL TRIGGER: /contact_preference_call/trigger/<user_id> (ALSO TIME-RESTRICTED)
    # --------------------------------------------------------------
    @app.route('/contact_preference_call/trigger/<user_id>', methods=['POST'])
    def contact_preference_call_trigger(user_id):
        allowed, server_time = is_within_call_window()
    ...
    # --------------------------------------------------------------
    @app.route('/debug-contact-preference-call')
    def debug_contact_preference_call():
        allowed, now = is_within_call_window()
        return jsonify({
            "debug": "contact_preference_call.py loaded",
