...
def schedule_booking_confirmed_call(app):
...
def make_booking_confirmed_call(app, phone: str, user_doc: dict, user_object_id, shift: dict):
    ...
    try:
        with app.app_context():
            ...
            socket_uri = os.getenv('SOCKET_URI_BOOKING_CONFIRMED')
...
def register_booking_confirmed_call_routes(app):
    @app.route('/api/register11', methods=['POST'])
    def api_register():
        try:
    ...
    @app.route("/call/completed11", methods=["POST"])
    def call_completed11():
