...
import { userSubTypesApi as api } from '../services/api'

function Modal({ item, userTypes, onClose, onSave, saving }) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-md mx-4">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <h2 className="text-base font-semibold text-gray-900">{isEdit ? 'Edit Sub Type' : 'Add Sub Type'}</h2>
        ...
        <div className="px-6 py-5 space-y-4">
          <div>
            ...
            <input className="input" placeholder="e.g. ICU Nurse" value={form.name}
              onChange={e => set('name', e.target.value)} autoFocus />
          </div>
          <div>
            <label className="block text-xs font-medium text-gray-500 mb-1">User Type</label>
            <select className="input" value={form.user_type_id} onChange={e => set('user_type_id', e.target.value)}>
        ...
        <div className="px-6 py-4 border-t border-gray-100 flex justify-end gap-2">
          ...
          <button onClick={() => onSave(form)} disabled={saving || !form.name.trim() || !form.user_type_id}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-sm mx-4">
        <div className="px-6 py-5 border-b border-gray-100">
          <h2 className="text-base font-semibold text-gray-900">Delete Sub Type</h2>
        </div>
...
export default function UserSubTypesPage() {
  ...
  const load = async () => {
    ...
    try {
      const [{ data: listData }, { data: utData }] = await Promise.all([api.list(), api.userTypes()])
      setItems(listData.data || [])
      setUserTypes(utData.data || [])
  ...
  const filtered = filterType ? items.filter(i => i.user_type_id === filterType) : items
  ...
  return (
    <div className="p-8 max-w-3xl">
      <div className="flex items-center gap-2 text-xs text-gray-400 mb-1">
        ...
        <span>User Sub Types</span>
      </div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">User Sub Types</h1>
          <p className="text-sm text-gray-500 mt-1">{filtered.length} sub type{filtered.length !== 1 ? 's' : ''}</p>
        </div>
      ...
      <div className="card mb-5 p-4 flex gap-3 items-center">
        <label className="text-sm text-gray-500 whitespace-nowrap">Filter by User Type</label>
        <select className="input flex-1" value={filterType} onChange={e => setFilterType(e.target.value)}>
          <option value="">All</option>
          {userTypes.map(ut => <option key={ut.id} value={ut.id}>{ut.name}</option>)}
        </select>
        {filterType && (
          <button onClick={() => setFilterType('')} className="text-xs text-gray-400 hover:text-gray-600">Clear</button>
        )}
      </div>
        ...
        <table className="w-full text-sm">
          <thead className="bg-gray-50 border-b border-gray-100">
            <tr>
              ...
              <th className="px-5 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wide">User Type</th>
          ...
          <tbody className="divide-y divide-gray-50">
            {loading ? (
              ...
              <tr><td colSpan={4} className="px-5 py-10 text-center text-gray-400">No sub types found</td></tr>
            ) : filtered.map(item => (
              <tr key={item.id} className="hover:bg-gray-50/50">
                <td className="px-5 py-3.5 font-medium text-gray-900">{item.name}</td>
                <td className="px-5 py-3.5">
                  {item.user_type ? (
                    <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-blue-50 text-blue-700">{item.user_type}</span>
      ...
      {(showAdd || editItem) && (
        <Modal item={editItem} userTypes={userTypes} onClose={() => { setShowAdd(false); setEditItem(null) }} onSave={handleSave} saving={saving} />
      )}
