    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-sm mx-4">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <h2 className="text-base font-semibold text-gray-900">{isEdit ? 'Edit Gender' : 'Add Gender'}</h2>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-sm mx-4">
        <div className="px-6 py-5 border-b border-gray-100">
          <h2 className="text-base font-semibold text-gray-900">Delete Gender</h2>
        </div>
        <div className="px-6 py-5">
...
export default function GendersPage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      setItems(data.data || [])
    } catch { setError('Failed to load genders') }
  ...
  const handleSave = async (form) => {
    setSaving(true)
    try {
      if (editItem) { await api.update(editItem.id, form); showToast('Gender updated'); setEditItem(null) }
      else          { await api.create(form);               showToast('Gender created'); setShowAdd(false) }
      load()
  ...
  const handleDelete = async () => {
    setDeleting(true)
    try { await api.delete(deleteItem.id); showToast('Gender deleted'); setDeleteItem(null); load() }
  ...
  return (
    <div className="p-8 max-w-xl">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Genders</h1>
          <p className="text-sm text-gray-500 mt-1">{items.length} gender{items.length !== 1 ? 's' : ''}</p>
        </div>
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-gray-50">
            {loading ? (
              ...
              <tr><td colSpan={2} className="px-5 py-10 text-center text-gray-400">No genders found</td></tr>
            ) : items.map(item => (
