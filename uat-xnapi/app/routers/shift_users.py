...
async def assign_staff_to_shift(request: Request, payload: AssignStaffRequest):
    ...
    # Build upstream payload — include session_user_id only when resolved
    upstream_json: dict = {"shift_id": xn_shift_id, "staff_id": xn_user_id}
    if resolved_session_user_id:
        upstream_json["session_user_id"] = resolved_session_user_id
    ...
    logger.info(
        f"[assign] upstream={upstream_url} shift_id={xn_shift_id} staff_id={xn_user_id}"
        f" session_user_id={resolved_session_user_id}"
    )
    ...
    return {
        ...
        "session_user_id":   resolved_session_user_id,
    }
...
async def run_outreach_orchestrator(
    ...
    # ── Normalise channel (accept any case: sms → SMS, phone → Phone etc.) ────
    _channel_map = {
    ...
    # ── Resolve sequence_id ──────────────────────────────────────────────────
    resolved_sequence_id: Optional[str] = None
    sequence_name: str = "—"
    ...
    if sequence_id and ObjectId.is_valid(sequence_id):
        ...
        if seq_doc:
            ...
            sequence_name = seq_doc.get("name", "—")
        else:
            steps.append({
    ...
    if not resolved_sequence_id:
        seq_doc = await db["sequences"].find_one(
            {"is_active": True},
            sort=[("sort_order", 1)],
        )
        if not seq_doc:
        ...
        resolved_sequence_id = str(seq_doc["_id"])
        sequence_name = seq_doc.get("name", "—")

    seq_oid = ObjectId(resolved_sequence_id)

    # ─────────────────────────────────────────────────────────────────────────
    # STEP 1 — shift-users/list  (filtered by Ops region county group)
    # ─────────────────────────────────────────────────────────────────────────

    # ── Ops region definitions ────────────────────────────────────────────────
    # Each Ops group covers a set of Irish counties. When the shift's county
    # belongs to one of these groups, we restrict the candidate list to users
    # whose county_id is in the same group (nearest county logic).
    OPS_GROUPS: dict = {
    ...
    if shift_county_name:
        for ops_name, county_names in OPS_GROUPS.items():
            for cn in county_names:
                if cn.lower() == shift_county_name.lower():
                    shift_ops_group  = ops_name
                    ops_county_names = county_names
                    break
            if shift_ops_group:
                break

    # Look up county _id values from the county collection for the matched group
...
async def run_group_outreach_orchestrator(
    ...
    # ── Normalise channel ─────────────────────────────────────────────────────
    _channel_map = {"phone": "Phone", "whatsapp": "WhatsApp", "email": "Email", "sms": "SMS"}
    channel = _channel_map.get(channel.lower(), "Phone")
    ...
    if sequence_id and ObjectId.is_valid(sequence_id):
        ...
        if seq_doc:
            ...
            steps.append({"step": "sequence_resolve",
                          "warning": f"sequence_id {sequence_id} not found — falling back"})
    ...
    # ── Ops region county filter (using first shift's county) ─────────────────
    OPS_GROUPS: dict = {
        "Ops 1": ["Dublin", "Kildare", "Laois", "Louth", "Meath", "Monaghan", "Offaly", "Westmeath"],
        "Ops 2": ["Cork", "Waterford", "Kerry", "Limerick", "Tipperary", "Clare"],
        "Ops 3": ["Galway", "Sligo", "Mayo", "Donegal", "Leitrim", "Longford", "Roscommon", "Cavan"],
