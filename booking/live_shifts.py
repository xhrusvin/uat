...
Supports:
  • Text search  (name, location, client, shift ID)
  • Date filter
  • Status filter
...
def _build_pipeline(search: str, date_filter_str: str, status_filter: str):
    ...
    Return a MongoDB aggregation pipeline that:
      1. Optionally filters by date
      2. Joins the clients collection to get client_name
      3. Optionally filters by free-text and/or status
    """
    base_match = {}
    ...
    pipeline = [
        {"$match": base_match},
        # Convert string client_id → ObjectId for $lookup
        {
            "$addFields": {
                "client_id_oid": {
                    "$cond": {
    ...
    # Text + client search (post-lookup so client_name is available)
    or_clauses = []
    if search:
    ...
    combined.update(status_match)

    if combined:
        pipeline.append({"$match": combined})

    # Keep only the fields we need in the template
    pipeline.append({
        "$project": {
            "_id": 1,
            "name": 1,
...
def _format_shifts(shifts_list: list) -> list:
    ...
    Post-process each shift document:
      ...
      • Guarantee client_name fallback
    """
...
def live_shifts():
    ...
    Query params:
      ...
      status_filter – one of VALID_STATUSES or blank for all
    """
...
def live_shifts_data():
    """
    JSON endpoint — returns the same data as live_shifts() but as JSON.
    Useful for AJAX polling / auto-refresh without a full page reload.
    """
    page = max(int(request.args.get("page", 1)), 1)
    search = request.args.get("search", "").strip()
    date_filter_str = request.args.get("date_filter", "").strip()
